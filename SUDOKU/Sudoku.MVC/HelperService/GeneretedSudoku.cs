...
public class GeneretedSudoku
{
	...
	private static bool BactTracking(int[,] grid, int[] guessArray)
	{
		int row = 0, col = 0;
	...
	private static bool FindEmptyLocation(int[,] grid, ref int row, ref int col)
	{
		for (row = 0; row < 9; row++)
			for (col = 0; col < 9; col++)
				if (grid[row, col] == 0)
	...
	private static bool UsedInRow(int[,] grid, int row, int value)
	{
		for (int i = 0; i < 9; i++)
			if (grid[row, i] == value)
				return true;
	...
	private static bool UsedInCol(int[,] grid, int col, int value)
	{
		for (int i = 0; i < 9; i++)
			if (grid[i, col] == value)
				return true;
	...
	private static bool UsedInBox(int[,] grid, int boxStartRow, int boxStartCol, int value)
	{
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
	...
	private static bool IsSafe(int[,] grid, int row, int col, int value)
	{
	...
	private static void MakeUnique(int[,] grid)
	{
	...
	private static void CheckUniqueness(int[,] grid, int[] guessArray, ref int number)
	{
		int row = 0, col = 0;
